...
int main() {
  ...
  auto f = [&](int i, int j) {
    return a[i] * b[j] * cpw[1LL * i * i * j % (md - 1) * j * j % (md - 1)];
  };
  Mint ans = 0;
  for (int i : si) {
    for (int j = 0; j < m; j++) {
  ...
  vector<vector<int>> crt(X, vector<int>(Y, -1));
  for (int i = 0; i < X * Y; i++) {
    crt[i % X][i % Y] = i;
  }
  for (int pi = 0; pi < 2; pi++) {
    for (int pj = 0; pj < 2; pj++) {
      vector<Mint> p(1 << 19);
      for (int i = 0; i < n; i++) {
        if (i % 2 == pi && i % X != 0 && i % Y != 0) {
          p[(invpX[1LL * i * i % X] << 10) + invpY[1LL * i * i % Y]] += a[i];
        }
      }
      vector<Mint> q(1 << 19);
      for (int j = 0; j < m; j++) {
        if (j % 2 == pj && j % X != 0 && j % Y != 0) {
          q[(invpX[1LL * j * j * j % X] << 10) + invpY[1LL * j * j * j % Y]] += b[j];
        }
      }
      vector<Mint> pq = p * q;
