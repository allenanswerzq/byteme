/* created   : 2021-08-14 11:30:55
 * accepted  : 2021-08-14 17:06:36
 */
#include <bits/stdc++.h>
using namespace std;
...
void solve() {
  ...
  int j = 0;
  for (int i = 0; i < N; i++) {
    while (j < N && seg.query(1, M).mn == 0) {
      seg.modify(A[j][1], A[j][2], +1);
      j++;
    }
    if (seg.query(1, M).mn > 0) {
      // The interval [1..M] is covered by a subset.
      trace(i, j, A[j - 1], A[i]);
      ans = min(ans, A[j - 1][0] - A[i][0]);
      assert(ans >= 0);
    }
    seg.modify(A[i][1], A[i][2], -1);
    seg.debug();
  }
  cout << ans << "\n";
}
